...
<body>
		...
		<div class="main-content">
			...
			<div class="page-content page-width">
				<div class="page-title">
					<h2>HSI Usage report per month</h2>

					<div class="breadcrumb-wrapper">
						<ol class="breadcrumb">
							<li><a href="./">Home</a></li>
							<li>HSI Usage report per month</li>
						</ol>
					</div>
				</div>
													...
													<div class="panel panel-default ">
														<div class=" panel-body ">
															<div class="panel-group">
																<div class="panel panel-default">
																	<div class="panel-body">
																		<form name="aaaUsage_Form" id="aaaUsagePerMonth_Form" action="getTotalAAAUsagePerMonth" method="POST">
																			<div class="row">
																				<div class="col-lg-12">
																					<div class="col-sm-3">
															...
															<div class="panel-body">
																<div class="col-sm-12">
																	<table class="table table-alt" >
																		...
																		<c:if test="${monthlySummary.preHrCount != 'NA'}">
																		 <th>Previous SixHour Live Customers</th>
																		 </c:if>
																		</tr>
																		</thead>
																		<tbody>
																			<tr>
																				<c:if test="${ not empty monthlySummary}">
																					...
																					<c:if test="${monthlySummary.preHrCount != 'NA'}">
																					<td>${monthlySummary.preHrCount}</td>
																					</c:if> 
																				</c:if>
																			</tr>
																		</tbody>
														...
														<div class="panel panel-default">
															<div class="panel-body">
																<div class="col-sm-12">
																	<table class="table table-alt" id="hsiPerMonthAndHourTableId">
																		<thead>
																		<tr>
																		<th>Year</th>
																		<th>Date-Month</th>
																		...
																		<tbody>
																			<c:forEach items="${list}"var="monthlySummary" varStatus="count">
													                            ...
													                            <!--<td><a data-toggle="modal" href=#
																						data-target="#myModal" class="viewHr"> View
																							Hour Wise Records</a></td> -->
																							<td><a data-toggle="modal" href=#
																						data-target="#myModal1" class="view6Hr" > View
																					...
																					<td hidden="hidden" class="1sub">${monthlySummary.sch01}</td>
																					<td hidden="hidden" class="2sub">${monthlySummary.sch02}</td>
																					<td hidden="hidden" class="3sub">${monthlySummary.sch03}</td>
																					<td hidden="hidden" class="4sub">${monthlySummary.sch04}</td>
																					<td hidden="hidden" class="5sub">${monthlySummary.sch05}</td>
																					<td hidden="hidden" class="6sub">${monthlySummary.sch06}</td>
																					<td hidden="hidden" class="7sub">${monthlySummary.sch07}</td>
																					<td hidden="hidden" class="8sub">${monthlySummary.sch08}</td>
																					<td hidden="hidden" class="9sub">${monthlySummary.sch09}</td>
																					<td hidden="hidden" class="10sub">${monthlySummary.sch10}</td>
																					<td hidden="hidden" class="11sub">${monthlySummary.sch11}</td>
																					<td hidden="hidden" class="12sub">${monthlySummary.sch12}</td>
																					<td hidden="hidden" class="13sub">${monthlySummary.sch13}</td>
																					<td hidden="hidden" class="14sub">${monthlySummary.sch14}</td>
																					<td hidden="hidden" class="15sub">${monthlySummary.sch15}</td>
																					<td hidden="hidden" class="16sub">${monthlySummary.sch16}</td>
																					<td hidden="hidden" class="17sub">${monthlySummary.sch17}</td>
																					<td hidden="hidden" class="18sub">${monthlySummary.sch18}</td>
																					<td hidden="hidden" class="19sub">${monthlySummary.sch19}</td>
																					<td hidden="hidden" class="20sub">${monthlySummary.sch20}</td>
																					<td hidden="hidden" class="21sub">${monthlySummary.sch21}</td>
																					<td hidden="hidden" class="22sub">${monthlySummary.sch22}</td>
																					<td hidden="hidden" class="23sub">${monthlySummary.sch23}</td>
																					<td hidden="hidden" class="24sub">${monthlySummary.sch24}</td>
																				</tr>
																			</c:forEach>
																		</tbody>
																	</table>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" id="hsiHourUsagePopUoDiv2">
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
